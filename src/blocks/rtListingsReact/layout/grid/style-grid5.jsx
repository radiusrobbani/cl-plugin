...
import { BadgeContainer, ListingContainer, ListingItemContainer, PriceContainer2, SoldContainer, ItemContentContainer } from '../../components/styles/Container.styles';
					...
					{data.map(listing => (
						<ListingItemContainer css={col_style} css_pad={col_padding} css_pm={promotion_style} className={[Object.values(listing.classes).join(" "), itemContainerClassNames]}>
							...
							{(content_visibility["thumbnail"] && listing.thumbnail) ? (
								<div className="listing-thumb">
									...
									{(content_visibility["price"] && listing.price) ? (
										<PriceContainer2 css={price_style} className="item-price listing-price" dangerouslySetInnerHTML={{ __html: listing.price }}></PriceContainer2>
									) : ''}
								</div>
							...
							<div class="rtcl-bottom">
								<ul>
									{content_visibility["author"] && listing.author_image ? (
										<li className="item-author"><div className="item-img" dangerouslySetInnerHTML={{ __html: listing.author_image }}></div></li>
									) : ''}

									<li className="action-btn">
										{content_visibility["favourit_btn"] && listing.favourite_link ? (
											<span className="rtcl-fav-wrap" dangerouslySetInnerHTML={{ __html: listing.favourite_link }}></span>
										) : ''}
										{content_visibility["quick_btn"] && listing.quick_view ? (
											<span className="rtcl-quick-wrap" dangerouslySetInnerHTML={{ __html: listing.quick_view }}></span>
										) : ''}
										{content_visibility["compare_btn"] && listing.compare ? (
											<span className="rtcl-compare-wrap" dangerouslySetInnerHTML={{ __html: listing.compare }}></span>
										) : ''}
									</li>
								</ul>
							</div>
