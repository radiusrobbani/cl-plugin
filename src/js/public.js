(function ($) {
	...
	$(document)
		.on('click', "#rtcl-resend-verify-link", function (e) {
			e.preventDefault();
	...
	$(function () {
		...
		$("#rtcl-checkout-form")
			.on('click', 'input[name="pricing_id"]', function (e) {
				if ($(this).val() == 0) {
		...
		$(".rtcl-media-upload-pp .rtcl-media-action")
			.on('click', 'span.add', function () {
				const addBtn = $(this);
				const ppFile = $("<input type='file' style='position:absolute;left:-9999px' />");
				$('body').append(ppFile);
				...
				ppFile.on('change', function () {
					...
					const allowed_image_types = rtcl.image_allowed_type.map(function (type) {
						return 'image/' + type;
					});
								...
								success: function (response) {
									...
									if (!response.error) {
										pp_wrap
											...
											.addClass('has-media')
											.parents('.rtcl-profile-picture-wrap')
											.find('.rtcl-gravatar-wrap')
											.hide();
		...
		$(document)
			.on('click', 'a.rtcl-favourites', function (e) {
				e.preventDefault();
				const _target = this,
					_self = $(_target),
					data = {
		 ...
		 * Slider Class.
		 */
		const RtclSlider = function ($target, args) {
			this.$target = $target;
		...
		function generate_list() {
			let type = listObj.active,
				items = listObj[type].items,
				ul = get_list(items);
			let container = $('<div class="rtcl-ui-select-list-wrap"><h4>' + listObj[type].text + '</h4><div class="rtcl-select-action"></div><div class="rtcl-ui-select-list"></div></div>');
		...
		$('.reveal-phone').on('click', function (e) {
			...
			if (!$this.hasClass('revealed')) {
				...
				if (options.safe_phone && options.phone_hidden) {
					const purePhone = options.safe_phone.replace(rtcl.phone_number_placeholder, options.phone_hidden);
				...
				if (options.safe_whatsapp_number && options.whatsapp_hidden) {
					const pureWPhone = options.safe_whatsapp_number.replace(rtcl.phone_number_placeholder, options.whatsapp_hidden);
	...
	if ($.fn.validate) {
		...
		$('#rtcl-lost-password-form, #rtcl-password-reset-form').validate();
				...
				if ($form.hasClass('rtcl-ajax-login')) {
					...
					$.ajax({
						...
						error: function () {
							$form
								.rtclUnblock()
								.append('<div class="rtcl-error alert alert-danger" role="alert"><p>' + rtcl_validator.messages.server_error + '</p></div>');
