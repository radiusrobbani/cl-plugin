(function ($) {
    ...
    $("#input-new-type-form").on('submit', function (e) {
        ...
        if (type) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    var alert_type = res.success ? 'alert-success' : 'alert-danger',
                        alert = $('<div class="col-12 alert alert-dismissible fade show" role="alert" />')
                            .append(res.message);
                ...
                error: function (e) {
                    $('<div class="col-12 alert alert-danger alert-dismissible fade show" role="alert" />')
                        .append('Server Error !!!').appendTo(form);
                    button.removeClass('spinner rtcl-loading').prop('disabled', false);
                }
            });
            ...
            $('<div class="alert alert-danger alert-dismissible fade show" role="alert" />')
                .append('Please fill type.').appendTo(form);
            type_field.focus();
        }
    });
    ...
    $(document).on('submit', '.input-update-type-form', function (e) {
            ...
            if (id && name && old_id) {
                ...
                $('<div class="col-12 alert alert-danger alert-dismissible fade show" role="alert" />')
                    .append('Please fill id and type.').appendTo(form);
            }
        });
    ...
    function generateTypeHtml(type) {
        var li = '<li class="list-group-item listing-type" data-id="' + type.id + '">' +
            '<div class="type-details d-flex">' +
            '<div class="type-info">' +
            '<div class="type-info-id">' + type.id + '</div>' +
            '<div class="type-info-name">' + type.name + '</div>' +
            '</div>' +
