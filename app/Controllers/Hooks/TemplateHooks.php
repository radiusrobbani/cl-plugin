...
class TemplateHooks
{
	static function init() {
		...
		add_action('rtcl_checkout_payment-receipt_endpoint', [__CLASS__, 'checkout_payment_receipt_endpoint'], 10, 2);
	 ...
	 * @param Filter $object
	 */
	static function widget_filter_form_category_item($object) {
	 ...
	 * @param Filter $object
	 */
	static function widget_filter_form_location_item($object) {
	 ...
	 * @param Filter $object
	 */
	static function widget_filter_form_price_item($object) {
	 ...
	 * @param Filter $object
	 */
	static function widget_filter_form_ad_type_item($object) {
		Functions::print_html($object->get_ad_type_filter(), true);
	}

	static function edit_account_form_submit_button() {
		?>
	...
	static function edit_account_form_location_field() {
	 ...
	 * @param Listing $listing
	 */
	static function listing_featured_badge($listing) {
	 ...
	 * @param Listing $listing
	 */
	static function listing_new_badge($listing) {
	...
	public static function listing_contact($post_id) {
		...
		if ($post_id) {
			$selected_locations = 'local' === apply_filters('rtcl_location_type', 'local') ? wp_get_object_terms($post_id, rtcl()->location, ['fields' => 'ids']) : [];
	...
	static function add_single_listing_review() {
	...
	static function catalog_ordering() {
		...
		if (Functions::get_loop_prop('is_search')) {
			$catalog_orderby_options = array_merge(array('relevance' => esc_html__('Relevance', 'classified-listing')), $catalog_orderby_options);
	...
	static function loop_item_meta_buttons() {
		Functions::get_template('listing/meta-buttons');
	}

	static function loop_item_badges() {
		global $listing;
		$listing->the_badges();
	}
	...
	static function logged_in_hidden_fields() {
	...
	static function registration_hidden_fields() {
	...
	static function edit_account_form_hidden_field() {
