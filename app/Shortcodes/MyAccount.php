...
class MyAccount
{
    ...
    public static function my_listings() {
        ...
        $rtcl_query = new \WP_Query(apply_filters('rtcl_my_listings_args', $args));
    ...
    public static function lost_password() {
        ...
        if (!empty($_GET['reset-link-sent'])) {
            ...
            if (isset($_COOKIE['wp-resetpass-' . COOKIEHASH]) && 0 < strpos($_COOKIE['wp-resetpass-' . COOKIEHASH], ':')) {
                list($rp_id, $rp_key) = array_map([Functions::class, 'clean'], explode(':', wp_unslash($_COOKIE['wp-resetpass-' . COOKIEHASH]), 2));
                ...
                if (is_object($user)) {
                    Functions::get_template('myaccount/form-reset-password', array(
                        'key'   => $rp_key,
                        'login' => $rp_login,
                    ));
                    return;
