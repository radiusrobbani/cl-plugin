...
class Functions
{
	 ...
	 * @return boolean
	 */
	static function is_wc_activated() {
	 ...
	 * @return bool
	 */
	static function is_pricing_disabled() {
	 ...
	 * @return bool
	 */
	static function is_price_type_disabled() {
	 ...
	 * @return bool
	 */
	static function is_gallery_image_required() {
	 ...
	 * @return bool
	 */
	static function is_video_urls_disabled() {
	 ...
	 * @return bool
	 */
	static function is_video_gallery_disabled() {
	 ...
	 * @deprecated
	 */
	static function get_formatted_price($price) {
	 ...
	 * @return string
	 */
	static function format_price_range($from, $to) {
		/* translators: 1: price from 2: price to */
		$price = sprintf(_x('<div class="rtcl-price-range">%1$s &ndash; %2$s</div>', 'Price range: from-to', 'classified-listing'), is_numeric($from) ? self::price($from) : $from, is_numeric($to) ? self::price($to) : $to);
		return apply_filters('rtcl_format_price_range', $price, $from, $to);
	}
	 ...
	 * @return string
	 */
	static function price($price, $only_formatted_price = false, $args = []) {
		$args = apply_filters(
			'rtcl_price_args',
			wp_parse_args(
				$args,
		 ...
		 */
		$price = apply_filters('raw_rtcl_price', $negative ? $price * -1 : $price, $original_price);
		...
		$formatted_price = ($negative ? '-' : '') . sprintf($args['price_format'], '<span class="rtcl-price-currencySymbol">' . self::get_currency_symbol($args['currency']) . '</span>', $price);
	...
	static function get_payment_formatted_price($price) {
	...
	static function get_payment_formatted_price_html($price) {
		$original_price = $price;
		$formatted_price = self::get_payment_formatted_price($original_price);
		$currency_symbol = self::get_currency_symbol(null, true);
		$price_format = self::get_price_format(true);

		$formatted_payment_price_html = apply_filters('rtcl_formatted_payment_price_html',
			sprintf($price_format, '<span class="rtcl-price-currencySymbol">' . $currency_symbol . '</span>', $formatted_price),
			$price_format, $currency_symbol, $formatted_price, $original_price);
	 ...
	 * @return string
	 */
	static function trim_zeros($price, $payment = false) {
	 ...
	 * @since  1.5
	 */
	static function get_price_decimals() {
	...
	static function get_sub_terms_filter_html($args, $terms = []) {
		...
		if (!empty($terms)) {
			...
			if ($html && rtcl()->location === $args['taxonomy']) {
				$html .= '<li class="is-opener"><span class="rtcl-more"><i class="rtcl-icon rtcl-icon-plus-circled"></i><span class="text">' . esc_html__("Show More",
	 ...
	 * @return int[]
	 */
	static function get_cf_ids($attr = []) {
	 ...
	 * @return int[]
	 */
	static function get_cfg_ids($attr = []) {
		$attr = wp_parse_args($attr, [
			'category_ids'         => false,
	 ...
	 * @return int[]
	 */
	static function get_cf_ids_by_cfg_id($group_id) {
		$args = [
			'post_type'        => rtcl()->post_type_cf,
			'post_status'      => 'publish',
			'posts_per_page'   => -1,
	...
	static function get_custom_field_html($field_id, $post_id = null) {
		...
		if ($field_id && $field) {
			...
			switch ($field->getType()) {
				case 'text':
					$field_html = sprintf('<input type="text" class="rtcl-text form-control rtcl-cf-field" id="%s" name="rtcl_fields[_field_%d]" placeholder="%s" value="%s"%s />',
						$id,
					...
					break;
				case 'textarea' :
					$field_html = sprintf('<textarea class="rtcl-textarea form-control rtcl-cf-field" id="%s" name="rtcl_fields[_field_%d]" rows="%d" placeholder="%s"%s>%s</textarea>',
						$id,
						absint($field->getFieldId()),
				...
				case 'select' :
					...
					$field_html = sprintf('<select name="rtcl_fields[_field_%d]" id="%s" class="rtcl-select2 rtcl-cf-field form-control"%s>%s</select>',
						absint($field->getFieldId()),
						$id,
						$required_attr,
						$options_html
				...
				case 'checkbox' :
					...
					if (!empty($choices)) {
						...
						foreach ($choices as $key => $choice) {
							...
							$check_options .= sprintf('<div class="form-check"><input class="form-check-input rtcl-cf-field" id="%s" type="checkbox" name="rtcl_fields[_field_%d][]" value="%s"%s><label class="form-check-label" for="%s">%s</label></div>',
								$id . $key,
								absint($field->getFieldId()),
								$key,
								$_attr,
				...
				case 'radio' :
					...
					if (!empty($choices)) {
						foreach ($choices as $key => $choice) {
							...
							$check_options .= sprintf('<div class="form-check"><input class="form-check-input rtcl-cf-field" id="%s" type="radio" name="rtcl_fields[_field_%d]" value="%s"%s><label class="form-check-label" for="%s">%s</label></div>',
								$id . $key,
								absint($field->getFieldId()),
								$key,
								$_attr,
					...
					break;
				case 'number':
					$field_html = sprintf('<input type="number" class="rtcl-number form-control rtcl-cf-field" id="%s" name="rtcl_fields[_field_%d]" placeholder="%s" value="%s" step="%s" min="%d" %s%s />',
						$id,
						absint($field->getFieldId()),
						esc_attr($field->getPlaceholder()),
						esc_attr($value),
					...
					break;
				case 'url':
					$field_html = sprintf('<input type="url" class="rtcl-url form-control rtcl-cf-field" id="%s" name="rtcl_fields[_field_%d]" placeholder="%s" value="%s"%s />',
						$id,
						absint($field->getFieldId()),
						esc_attr($field->getPlaceholder()),
						esc_url($value),
				...
				case 'date':
					...
					$field_html = sprintf('<input type="text" class="rtcl-date form-control rtcl-cf-field%s" id="%s" name="rtcl_fields[_field_%d]" placeholder="%s" data-options="%s" value="%s"%s />',
			...
			if (isset($_REQUEST['is_admin']) && $_REQUEST['is_admin'] == 1) {
				...
				$html .= sprintf('<div class="form-group rtcl-cf-wrap"%s>
...
//            ]));
	...
	public static function locate_template($template_name, $template_path = '', $default_path = '') {
		...
		$template_files = [];
		// Legacy template supporget_sub_termst
		if (false !== strpos($template_name, 'listings/single/') || false !== strpos($template_name, 'listings/')) {
			$legacy_name = $template_name;
			$template_name = str_replace(['listings/single/', 'listings/'], ['listing/', 'listing/'], $template_name);
	...
	static function get_template_part($slug, $name = '') {
		$cache_key = sanitize_key(implode('-', array('template-part', $slug, $name, RTCL_VERSION)));
	 ...
	 * @throws \Exception
	 */
	static function rtcl_payment_completed($order, $data = array()) {
	 ...
	 */
	public static function create_new_user($email, $username = '', $password = '', $args = array(), $source = null) {
	 ...
	 * @return void
	 */
	static function add_default_expiry_date(int $listing_id) {
