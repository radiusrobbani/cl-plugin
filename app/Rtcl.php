...
//for gutenberg
use Rtcl\Controllers\GutenbergController;
...
if (!class_exists(Rtcl::class)) {
	...
	final class Rtcl
	{
		 ...
		 * @see LoggerInterface
		 */
		public function logger($logLevelThreshold = LogLevel::DEBUG, $options = array())
		{
			static $logger = null;
		 ...
		 * @param null|bool $ssl     If you should use SSL, null if should auto-detect. Default: null.
		 *
		 * @return string
		 */
			...
			if (!$this->cart) {
				...
				if (is_object($cart_class)) {
					...
					if (class_exists($cart_class)) {
						...
						) ? call_user_func([$cart_class, 'instance']) : new $cart_class();
					}
				}
			}
		...
		{
			if (isset($_GET['rtcl-type']) && in_array(
				$_GET['rtcl-type'],
				[
					'success',
					'error',
