...
class Listing extends Data
{
    ...
    function can_show_price() {
        ...
        $can_show_price = ((!empty($this->moderation_settings[$display_option]) && !in_array('price', $this->moderation_settings[$display_option])) || Functions::is_price_disabled() || $this->get_pricing_type() === 'disabled') ? false : true;
     ...
     */
    function badges() {
        return Functions::get_template_html("listing/badges");
    }
    ...
    public function get_price_html() {
        ...
        if ('' !== $min_price) {
            if ($this->get_pricing_type() === 'range' && '' !== $max_price && $min_price !== $max_price) {
                $price = Functions::format_price_range($min_price, $max_price);
            } else {
                $price = Functions::price($min_price);
            }
        ...
        $price_html_format = apply_filters('rtcl_get_price_html_format', '%1$s%2$s');
        $price_html = sprintf($price_html_format, $price, $price_meta_html);
    ...
    public function get_price_suffix($price = '') {
        ...
        if ($suffix && wc_tax_enabled() && 'taxable' === $this->get_tax_status()) {
            ...
            $replacements = array(
                '{price_including_tax}' => wc_price(wc_get_price_including_tax($this, array(
                    'qty'   => $qty,
                    'price' => $price
                ))),
                ...
                '{price_excluding_tax}' => wc_price(wc_get_price_excluding_tax($this, array(
                    'qty'   => $qty,
                    'price' => $price
                ))),
    ...
    function user_contact_location_at_single() {
        $locations = [];
        $location_type = apply_filters('rtcl_location_type', 'local');
